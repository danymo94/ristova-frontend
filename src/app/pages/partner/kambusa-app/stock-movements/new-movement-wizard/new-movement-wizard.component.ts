  ...
  export class NewMovementWizardComponent implements OnInit, OnDestroy {
    ...
    constructor() {
      ...
      effect(() => {
        ...
        if (error && this.submitting) {
          ...
          this.toastService.showError('Errore nella creazione del movimento: ' + error);
          return;
        }
    ...
    onProductSelect(event: any, index: number) {
      ...
      if (this.selectedType === StockMovementType.INVENTORY && this.warehouse) {
        const balance = this.warehouseStore.warehouseBalance();
        if (balance && balance.items) {
          const item = balance.items.find((i) => i.rawProductId === product.id);
          if (item) {
            this.products.at(index).patchValue({
              expectedQty: item.currentQuantity,
              actualQty: item.currentQuantity, // Default, l'utente può cambiare
            });
      ...
      try {
        ...
        switch (typeValue.movementType) {
          case StockMovementType.PURCHASE:
            this.createInboundMovement(projectId, warehouseId, {
          ...
          case StockMovementType.WASTE:
            this.createOutboundMovement(projectId, warehouseId, {
          ...
          case StockMovementType.TRANSFER:
            ...
            this.createTransferMovement(projectId, {
          ...
          case StockMovementType.INVENTORY:
            ...
            this.createInventoryCheck(projectId, warehouseId, {
    ...
    private createInboundMovement(
      ...
      // Non abbiamo più bisogno di sottoscriverci - usiamo l'effect per gestire il risultato
    }
  
    private createOutboundMovement(
      projectId: string,
      warehouseId: string,
